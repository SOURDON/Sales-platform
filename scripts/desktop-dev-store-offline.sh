#!/usr/bin/env bash
# Запуск офлайн-магазина Fotografy Store 1.1.3 (dev, Mac).
...
if [[ ! -f "$REPO/desktop/.env.store-offline" ]]; then
  echo "Не найден desktop/.env.store-offline" >&2
  exit 1
fi

if [[ ! -f "$REPO/desktop/src-tauri/icons-store/icon.icns" ]]; then
  echo "Генерируем иконки офлайн-магазина…"
  (cd "$REPO/desktop" && npm run icon:store)
fi
...
pkill -f "tauri dev --config src-tauri/tauri.store.conf.json" 2>/dev/null || true
pkill -f "target/debug/Fotografy" 2>/dev/null || true
sleep 1

export DESKTOP_BUILD_PROFILE=store-offline
echo ""
echo "=== Fotografy Store 1.1.3 (офлайн-магазин) ==="
echo "Это окно можно свернуть. Закрытие окна остановит приложение."
echo ""

cd "$REPO/desktop"
exec npm run dev:store
